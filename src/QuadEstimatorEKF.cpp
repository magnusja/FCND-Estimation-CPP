...
void QuadEstimatorEKF::UpdateFromIMU(V3F accel, V3F gyro)
{
  ...
  auto integratedQ = q.IntegrateBodyRate(gyro, dtIMU);

  float predictedPitch = integratedQ.Pitch();
  float predictedRoll = integratedQ.Roll();
  ekfState(6) = integratedQ.Yaw();
...
void QuadEstimatorEKF::UpdateFromMag(float magYaw)
{
  ...
  if(d < -F_PI) zFromX(0) -= 2 * F_PI;
  else if(d > F_PI) zFromX(0) += 2 * F_PI;
